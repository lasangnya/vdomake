...
/**
 * Keyframe payload as submitted by the client (no DB-generated id/projectId).
 * Defined explicitly rather than via `.omit()` on the keyframe schema —
 * Zod 4 forbids `.omit()` on schemas containing refinements (superRefine).
 */
export const keyframeInputSchema = z.object({
  sceneId: z.string().min(1),
  startTime: z.number().nonnegative(),
  endTime: z.number().positive(),
  transitionDuration: z.number().nonnegative().default(0.5),
