...
export const audioTracks = pgTable(
  ...
  (table) => [index('audio_tracks_project_id_idx').on(table.projectId)],
);
...
export const keyframes = pgTable(
  'keyframes',
  {
    ...
    transitionDuration: doublePrecision('transition_duration').notNull().default(0.5),
    isAutoGenerated: boolean('is_auto_generated').notNull().default(false),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => [
    index('keyframes_project_id_idx').on(table.projectId),
    index('keyframes_scene_id_idx').on(table.sceneId),
  ],
);
